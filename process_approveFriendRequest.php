...
$userID = $_POST['userID'];
	...
	require("/lib/sendgrid-php/sendgrid-php.php");
	 ...
	 echo "<script type='text/javascript'>alert('Are you sure you want to confirm this?')</script>";
	  echo "<script>setTimeout(\"location.href = 'approveFriend.php';\");</script>";

foreach( $userID as $key => $n ) {
    	  
	       
	        $q = "UPDATE friend SET status = 0 where fromUserID = '".$userID[$key]."' AND toUserID = ".$_SESSION['user'];
...
foreach( $userID as $key => $n ) {
	        ...
	        $q = "delete from friend where fromUserID = '".$userID[$key]."' AND toUserID = ".$_SESSION['user'];
   $res=mysqli_query($conn, "SELECT * FROM user WHERE userID='".$userID[$key]."'");
$userRow=mysqli_fetch_array($res);
