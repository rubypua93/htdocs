...
<?php
	...
	mysql_select_db("alumnidatabase");
		...
		$result = mysql_query("SELECT jobID,title,company,created_by,create_date,admin_ind FROM job  WHERE createUserID = '".$_SESSION['admin']."' AND admin_ind = 'Y' order by create_date desc");
		$number = 1;
	
		while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
			if($row['5'] == 'N'){
				echo "  <tr><td>".$number.".</td> 
				<td> <a href='jobEditAdmin.php?uid=".$row['0']."'> ".$row['1']." </a> </td><td>".$row['2']."</td><td>".$row['3']."</td> <td>".$row['4']."</td></tr>";
			}
			else{
				echo "  <tr><td>".$number.".</td> 
				<td> <a href='jobEditAdmin.php?uid=".$row['0']."'> ".$row['1']." </a> </td><td>".$row['2']."</td><td>Admin</td> <td>".$row['4']."</td></tr>";
			}
			$number = $number + 1;
		}
